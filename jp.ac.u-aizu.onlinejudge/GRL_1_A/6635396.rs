...
pub trait Push {
    ...
    fn pq_push(&mut self, x: Self::T);
}
pub trait Pop {
    type T;

    fn pq_pop(&mut self) -> Option<Self::T>;
}
...
// impl<T> NewEmpty for std::collections::BinaryHeap<T> {
...
impl<T: std::cmp::Ord> Push for std::collections::BinaryHeap<T> {
    ...
    fn pq_push(&mut self, x: Self::T) { self.push(x); }
}

impl<T: std::cmp::Ord> Pop for std::collections::BinaryHeap<T> {
    type T = T;

    fn pq_pop(&mut self) -> Option<Self::T> { self.pop() }
}
...
where
    E: To<V = usize> + Weight<u64>,
    F: Fn(&mut T, usize, &E),
    Q: Push<T = std::cmp::Reverse<(u64, usize)>>
    ...
    T: std::fmt::Debug,
{
    ...
    let mut dist = vec![None; n];
    dist[src] = Some(0);
    hq.pq_push(Reverse((0, src)));
    while let Some(Reverse((du, u))) = hq.pq_pop() {
        if du > dist[u].unwrap() {
            continue;
        }
        for e in sparse_graph[u].iter() {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // let res = dijkstra_sparse(g.edges(), r);
    let mut data = vec![None; n];
    data[r] = Some(0);
    let res = general_dijkstra_sparse::<
        _,
        _,
        _,
